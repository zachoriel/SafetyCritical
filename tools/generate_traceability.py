...
# Parse C#
cs_rows = json.loads(subprocess.check_output(["python", str(ROOT/"tools/parse_trx.py"), str(trx_latest)]))
# Parse Python
py_rows = json.loads(subprocess.check_output(["python", str(ROOT/"tools/parse_junit.py"), str(PY_JUNIT)]))
...
for row in py_rows:
    ...
    name = row['name']
    if name == 'test_normal_operation': req = 'REQ-005'
    elif name == 'test_emergency_flag_consistency': req = 'REQ-006'
    elif name == 'test_invalid_command_rejected': req = 'REQ-007'
    elif name == 'test_authorized_shutdown': req = 'REQ-004'
    elif name == 'test_subcool_trip': req = 'REQ-001'
    elif name.startswith('test_boundary_low_pressure'): req = 'REQ-002'
    elif name.startswith('test_boundary_high_temp'): req = 'REQ-003'
    else: req = None
    if req:
...
out_dir = ROOT / "artifacts"; out_dir.mkdir(exist_ok=True)
